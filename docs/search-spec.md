...
- 리스너: `PgUnifiedSearchIndexListener`
  - `EntityIndexEvent` 수신 → 도메인별 `SearchDocument` 변환 → `unified_search_index` upsert
  - upsert 후 `collapseByLink(link)` 로 동일 link 중복을 active 1건으로 정리(나머지 deactivate).
    canonical = 활성 행 중 최소 id(전부 비활성이면 최소 id) → reconcile 선택과 수렴.
- ES 리스너와 병행 운영 가능하도록 별도 빈으로 분리한다.

### 5.2.1 중복 처리(dedup)
- 동일 원문이 여러 카테고리 게시판에 게재되면 `notice` 테이블엔 서로 다른 row(originalId 다름),
  같은 `link` 로 존재 → 검색 중복 노출 위험.
- 정책: **link 동일 = 중복**. active 1건만 유지.
  - 실시간: 이벤트 리스너 `collapseByLink`.
  - 전체/정합성: `collectAll()` 이 link 기준 dedup 후 적재/reconcile.
- link 가 null/blank 면 식별 불가 → dedup 대상 제외(각각 독립 문서).
