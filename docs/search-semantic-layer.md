...
게시글이 아니라 구독 가능한 표준 Topic을 반환한다. 사용자가 후보를 선택하면 기존 키워드 등록 요청에
선택 필드 `topicId`를 함께 보낸다. 생략하면 기존 자유 키워드 매칭이 그대로 동작한다.
...
Topic 구독 저장, 신규 공지 계층 매칭, 활성화와 회원·공지 단위 중복 방지는 구현 완료했다.
대상·캠퍼스는 사용자가 별도 필터를 선택하는 UI가 생길 때 선택 필드로 확장한다. 재분류 운영자 API와
품질 대시보드는 운영 편의 기능이며 현재 검색·알림 동작의 필수 조건은 아니다.
